...
export class DatabaseStorage implements IStorage {
  ...
  // Retorna todos os usuários (técnicos e administradores) para notificações
  ...
  async updateTicketPriority(id: string, priority: string): Promise<void> {
    await db
      .update(tickets)
      .set({ priority, updatedAt: sql`datetime('now', '-3 hours')` })
      .where(eq(tickets.id, id));
  }
  ...
  async updateTicketStatus(ticketId: string, status: string): Promise<void> {
    ...
    if (status === 'resolved') {
      updateData.resolvedAt = sql`datetime('now', '-3 hours')`;
    }
  ...
  async assignTicket(id: string, technicianId: string): Promise<void> {
    await db
      ...
      .set({
        ...
        status: 'in_progress',
        acceptedAt: sql`datetime('now', '-3 hours')`,
        updatedAt: sql`datetime('now', '-3 hours')`
      })
      .where(eq(tickets.id, id));
  }
