...
Este documento contém instruções para desenvolvimento local e deploy em produção.

---

## 🚀 Desenvolvimento Local (Linux/Mac/Windows)
...
3. **Resolver problema de porta 5000 já em uso:**

Se você receber o erro `EADDRINUSE: address already in use 0.0.0.0:5000`, significa que outro processo está usando a porta 5000.

**Opção 1: Matar o processo na porta 5000 (Recomendado)**
```bash
# Descobrir qual processo está usando a porta 5000
sudo lsof -i :5000
...
```bash
# Editar package.json e mudar a porta no script dev
# OU adicionar no .env:
echo "PORT=3000" >> .env
```
...
O sistema estará disponível em `http://localhost:5000` (ou na porta configurada).

### Credenciais Padrão de Desenvolvimento
- **Admin:** `admin` / `admin123`
- **Técnico:** `tecnico1` / `tech123`

### Solução de Problemas Comuns

#### Login não persiste (tela pisca e volta ao login)

Este problema ocorre quando você roda o sistema localmente fora do ambiente Replit. As correções incluem:

1. **Garantir que NODE_ENV está configurado corretamente:**
```bash
# O script npm run dev já configura isso automaticamente
# Mas se estiver rodando manualmente:
...
2. **Limpar cookies do navegador:**
   ...
   - Limpe todos os cookies do localhost
...
## 📦 Guia de Deploy - Produção em Servidor Linux
